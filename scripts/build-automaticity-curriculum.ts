...
const registryPath = resolve(
  root,
  "shared/learning-core/content/construction-map.json",
);
...
if (Bun.argv.includes("--init")) {
  if (await Bun.file(registryPath).exists())
    throw new Error(
      "Construction map already exists; edit mappings explicitly instead of regenerating IDs.",
    );
  const mappings: ConstructionMapping[] = [];
  for (const [language, units] of [
    ["en", en],
    ["de", de],
  ] as const) {
    const families = assignments[language].split(",");
    if (families.length !== units.length)
      throw new Error(
        `${language}: ${families.length} mappings for ${units.length} units`,
      );
    units.forEach((unit, index) =>
      mappings.push({
        id: `${language}.c.${String(index + 1).padStart(3, "0")}`,
        language,
        lessonAlias: `${unit.level}::${unit.title}`,
        familyIds: [`G${families[index]!.padStart(2, "0")}` as FamilyId],
        prerequisites: [],
        review: "authored",
      }),
    );
  ...
  for (const mapping of mappings)
    mapping.prerequisites = (prerequisites[mapping.familyIds[0]!] ?? [])
      .map(
        (family) =>
          mappings.find(
            (row) =>
              row.language === mapping.language &&
              row.familyIds.includes(family as FamilyId),
          )?.id,
      )
      .filter((id): id is string => !!id && id !== mapping.id);
    ...
    JSON.stringify(
      {
        ...
        scopeStatus: "proposed-construction-crosswalk-human-review-pending",
        mappings,
      },
      null,
      2,
...
for (const [language, units, project] of [
  ["en", en, "Apps/English/English-Automaticity"],
  ...
  for (const mapping of registry.mappings.filter(
    ...
    const task = (
      ...
      prompt: string,
      answer: string | null,
      policy: PracticeTask["answerPolicy"],
      ...
      itemFamily: answer
        ? `${language}.item.${hash(answer.normalize("NFC").trim())}`
        : `${mapping.id}.${stage}.${index}`,
      contextId: `${mapping.id}.legacy-context.${stage}`,
      rubricVersion:
        policy === "closed" ? "closed-nfc-case-v1" : "open-review-v1",
      stage,
      modality,
      partition: stage === "notice" ? "teaching" : "practice",
      transferCondition: stage === "transfer" ? "target_named" : "none",
      contentReview: mapping.review,
      prompt,
      answerPolicy: policy,
      responseKind: kind,
      acceptedAnswers: policy === "closed" && answer ? [answer] : [],
      hints: [unit.rule],
      solution: answer,
      normalisation: {
        nfc: true,
      ...
      sourceId: `${language}:authored:${alias}`,
    });
    const tasks: PracticeTask[] = [
      task(
        ...
        0,
        language === "en"
          ? "Explain in your own words when this pattern is useful."
          : "Erkläre mit eigenen Worten, wann dieses Muster sinnvoll ist.",
        null,
        "reflection",
        "reflection",
      ),
    ...
    unit.exercises.forEach((exercise, index) => {
      ...
      const declaredOpen =
        metadata &&
        "answerRole" in metadata &&
        metadata.answerRole === "inspiration";
      const kind: PracticeTask["responseKind"] = /_{2,}|…|\.\.\./u.test(prompt)
        ? "cloze"
        : /^(correct|korrigiere|verbessere)\b/iu.test(prompt)
          ? "correction"
          : /^(choose|wähle|ergänze|complete)\b/iu.test(prompt)
            ? "choice"
            : /^(change|transform|wandle|setze)\b/iu.test(prompt)
              ? "transformation"
              : "free_output";
      ...
      if (policy === "closed" && metadata && "acceptedAnswers" in metadata)
        entry.acceptedAnswers.push(...(metadata.acceptedAnswers ?? []));
      tasks.push(entry);
    });
    const own =
      language === "en"
        ? `Use ${unit.title} to explain something from your own life. Keep the meaning clear. Different correct answers are possible.`
        : `Verwende ${unit.title}, um etwas aus deinem eigenen Alltag zu erklären. Achte auf eine klare Bedeutung. Verschiedene richtige Antworten sind möglich.`;
    tasks.push(
      task("produce", 0, own, null, "open", "free_output"),
      task("produce", 1, own, null, "open", "free_output", "speaking"),
    );
    tasks.push(
      task("vary", 99, unit.recallTest || own, null, "open", "free_output"),
    );
    tasks.push(
      task(
        "repair",
        99,
        unit.repairTest || unit.commonError,
        null,
        "open",
        "free_output",
      ),
    ...
    tasks.push(
      task(
        ...
        0,
        language === "en"
          ? `Without reopening the explanation, give a new example of ${unit.title} and explain its meaning.`
          : `Gib ohne erneute Erklärung ein neues Beispiel für ${unit.title} und erkläre seine Bedeutung.`,
        null,
        "open",
        "free_output",
      ),
    ...
    if (!tasks.some((entry) => entry.stage === "retrieve"))
      tasks.push(
        task(
          "retrieve",
          99,
    ...
    for (const stage of [
      ...
      if (
        !tasks.some(
          (entry) => entry.stage === stage && entry.modality === "speaking",
        )
      ) {
        const written = tasks.find(
          (entry) => entry.stage === stage && entry.modality === "writing",
        )!;
        const spoken = task(
          stage,
          98,
          (language === "en"
            ? "Say your answer aloud and record it. "
            : "Sprich deine Antwort laut und nimm sie auf. ") + written.prompt,
          written.solution,
          "open",
          "free_output",
          "speaking",
        );
        spoken.itemFamily = written.itemFamily;
        tasks.push(spoken);
      }
    }
    ...
    for (const stage of [
      ...
      for (const modality of ["writing", "speaking"] as const) {
        ...
        coverage.push({
          ...
          stage,
          modality,
          status: matching.length ? "authored" : "missing",
          taskIds: matching.map((task) => task.id),
          humanReview: "pending",
          evaluator: matching.some((task) => task.answerPolicy === "closed")
            ? "closed-nfc-case-v1"
  ...
  // Unchanged supplementary tasks retain their original task versions.
  const supplemental = await buildSupplementaryUnits(root, language, "2026-09-05.3");
  pack.units.push(...supplemental);
  await applyTaskRevisions(root, pack);
  for (const scope of REPRESENTATIVE_SCOPES.filter(row => row.rule.startsWith(language + "."))) {
    const unit = pack.units.find(row => row.id === scope.constructionId);
    if (!unit) throw new Error(`Missing representative construction ${scope.constructionId}`);
    // Prior definitions stay byte-equivalent. New tasks are first for actual Daily/Grammar selection.
    unit.tasks.unshift(...representativeTasks(scope));
  }
  // Replace the preliminary legacy cells with the current active task set.
  for (let index = coverage.length - 1; index >= 0; index--) if ((coverage[index] as {language:string}).language === language) coverage.splice(index, 1);
  for (const unit of pack.units) for (const stage of ["notice","retrieve","vary","produce","repair","transfer","retain"] as const)
    for (const modality of ["writing","speaking"] as const) {
      if (modality === "speaking" && ["en.c.124","de.c.156"].includes(unit.id)) continue;
      const matching = activePracticeTasks(unit).filter(task => task.stage === stage && task.modality === modality);
...
await output(
  ...
  JSON.stringify(
    coverage.map((row, index) => ({
      id: `COVER-${String(index + 1).padStart(4, "0")}`,
      ...(row as object),
      work: "Review construction, task semantics and assessment support; implement or author missing stage; verify on learner route.",
    })),
    null,
    2,
  ) + "\n",
