...
if (product !== "English" && product !== "German")
  throw Error("Choose English or German");
...
const receipt = {
  ...
  product,
  check,
  checkExit: null as number | null,
  packageExit: null as number | null,
  version: JSON.parse(
    await readFile(
      resolve(app, "distribution/windows-modern/setup.config.json"),
      "utf8",
    ),
  ).version,
};
...
try {
  receipt.checkExit = await run(check, "completion-release-check.log");
  if (receipt.checkExit === 0)
    receipt.packageExit = await run(
      "package:windows-exe",
  ...
  await writeFile(
    resolve(output, "completion-build-receipt.json"),
