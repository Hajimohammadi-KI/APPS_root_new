...
const releasePackager = await readFile(new URL("../scripts/package-windows-release.ps1", import.meta.url), "utf8");
...
test("release version is synchronized across package and Windows setup", () => {
  assert.equal(packageJson.version, "0.7.1-version2");
...
test("Windows release packager includes current source and excludes local state", () => {
  assert.match(packageJson.scripts["package:windows"], /package-windows-release\.ps1/);
  assert.match(releasePackager, /public\\prompts\\complete-daily-thesis-work-prompt\.md/);
  for (const excluded of ["node_modules", ".wrangler", ".env.local", "outputs", "releases"]) {
    assert.match(releasePackager, new RegExp(excluded.replace(".", "\\.")));
  }
  assert.match(releasePackager, /Security\.Cryptography\.SHA256/);
});
