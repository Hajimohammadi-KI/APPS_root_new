...
export async function saveConversationSession(
  ...
  try {
    await new Promise<void>((resolve, reject) => {
      ...
      transaction.onerror = () => reject(transaction.error ?? new Error("The session could not be saved."));
      transaction.onabort = () => reject(transaction.error ?? new Error("The save transaction was aborted."));
    });
  } finally {
    database.close();
  }
