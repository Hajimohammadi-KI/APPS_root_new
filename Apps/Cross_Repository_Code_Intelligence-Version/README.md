...
**CURRENT · Version2 0.6.3** — light-lavender, responsive interface. The Projekt-Fahrplan is integrated into the Projekt-Lernplan as a second view with one shared, clickable progress state. Legacy `/projekt-fahrplan` links redirect to the integrated view. The medically protected 25-week plan starts on **30 August 2026**: W1 runs before the first procedure, the first seven days after each procedure contain no work, and suitable software-engineering design tasks switch to a clearly marked paper-only mode during days 8–14. Digital focus is disabled on those paper days. Every article now states whether it must be read completely or only through named sections, and all 25 weeks expose at least one mandatory weekly output. The structured daily closing note uses full-width, comfortably sized writing areas. Catch-up sessions 1–7 remain optional from 19 October, at most one per week after the core output.
