...
/* eslint-disable @next/next/no-html-link-for-pages -- full reload keeps route-scoped settings styles isolated */
...
import type { PlatformStatusResponse } from "@cross-repo/contracts";
...
export default function SettingsApp() {
  ...
  return <main className={classNames}>
    <header className="topbar settings-topbar">
      <a className="brand" href="/settings"><span>E</span><div><b>{settings.labels.appName}</b><small>{settings.labels.appSubtitle}</small></div></a>
      <div className="header-status"><a className="header-link" href="/pdf-reader">PDF Visual</a><span className="secure">● {saveState}</span><a className="header-link" href="/">← Zum Lernplan</a></div>
    </header>
    <section className="settings-content">
      <section className="settings-platform-card" aria-label="Systemstatus" aria-live="polite">
        <div className="settings-platform-heading">
          ...
          <div><b>Gemeinsame Systemverbindung</b><small>PDF Visual, Einstellungen und Lernplan verwenden dieselben sicheren Dienste.</small></div>
        </div>
        <div className="settings-platform-items">
          ...
          <span className={`settings-platform-pill ${connections.google.connected ? "ok" : "waiting"}`}><b>Google Translation</b>{connections.google.connected ? "Verbunden" : "Nicht verbunden"}</span>
        </div>
