...
To prepare the whole curriculum, run `bun scripts/prepare-automaticity-content-review.ts --all NEW-directory`. It creates one packet per construction, a manifest of file hashes and a searchable HTML index. It refuses to reuse an existing directory, protecting completed reviewer work. The current set is [280 construction packets](../artifacts/content-review-packets/all-20260905-task-revision2/index.html), covering 3,906 required cells and 4,756 active tasks. All review fields remain blank. The packets also retain 506 retired task definitions for historical context; these are excluded from new task approval. Earlier packet sets remain available but their hashes do not qualify the revised content.
...
Evidence files now require the structured version 2 format in `contentEvidenceDraft` or `manualEvaluatorEvidenceDraft`. A nonempty text file is insufficient. Record `provenance: "human_review"` only after actual human review; identity, role, date and decision must match the ledger. The evidence must identify the exact cell, curriculum and mapping versions, and construction hash. Each task needs its exact task/rubric versions, explicit judgments and a specific note of at least 20 characters. Construction checks cover form, meaning, use, contrasts, alternatives, prerequisites and sources. Task checks cover target elicitation, stage, modality, answer policy, accepted answers and exposure. For an open task, reviewing accepted answers means confirming that it does not incorrectly use an exact-string answer list.

Evaluator evidence also pins its ID, version, kind, task IDs, rubrics and benchmark reference. Its task judgments cover criteria, alternatives, meaning preservation, abstention and exposure. An unresolved check blocks approval. A human assessment procedure belongs in the review note or a referenced procedure described there. The gate verifies recorded evidence and consistency; it cannot establish whether an unknown person actually performed a competent review. Synthetic test records exercise the schema only and must never be copied into the real ledger.
...
The FSRS module stays in shadow mode. `qualifyProspectiveReviews` requires consent recorded before the original delayed attempt and an explicit rating linked to its current qualified assessment. Only previously attempted retrieval/retention items from the practice partition enter this calculation. Exact task, version, context, item family, rubric and modality identify each memory item. New items, changed versions, transfer and held-out tasks cannot borrow another item's history. Missing, malformed, conflicting, premature and invalidated ratings are excluded; a recorded withdrawal stops calculation. Legacy aggregates never supply missing history. The old English/German score-to-rating collection calls have been removed, preserving historical bytes. `buildQualifiedFsrsCandidates` calculates candidate dates without writing learner due dates or enabling rollout. These functions have synthetic tests; no qualified prospective learner history or candidate-schedule experiment has been collected. Keep the baseline available for rollback.
