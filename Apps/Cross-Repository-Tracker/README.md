...
**CURRENT · Version2 0.6.7** — light-lavender, responsive interface with a verified 12 px minimum for all authored interface text. The Projekt-Fahrplan is integrated into the Projekt-Lernplan as a second view with one shared, clickable progress state. Legacy `/projekt-fahrplan` links redirect to the integrated view. The medically protected 25-week plan starts on **30 August 2026**: W1 runs before the first procedure, the first seven days after each procedure contain no work, and suitable software-engineering design tasks switch to a clearly marked paper-only mode during days 8–14. Digital focus is disabled on those paper days. Every article now states whether it must be read completely or only through named sections, and all 25 weeks expose at least one mandatory weekly output. Every one of the 146 plan days also starts with one or two authoritative prerequisite learning links, the exact section to read, a time estimate, and an instruction for applying it to that day's artefact. Each day now contains a collapsible step-by-step study guide that turns those sources into the same concrete 4-hour flow: exact questions, a maximum of 12–15 Zotero vocabulary annotations, targeted rereading, closed-book recall, a two-minute English explanation, and an Artefact + Test + Evidence completion gate. Paper-only days automatically replace Zotero and screen steps with equivalent written work. On medically cleared regular days the default mode is four hours: the prerequisite reading is part of the first 70-minute block, the three task blocks total 210 minutes, and two 15-minute breaks complete the four-hour window. The structured daily closing note uses full-width, comfortably sized writing areas. Catch-up sessions 1–7 remain optional from 19 October, at most one per week after the core output. The Windows installation now checks a public HTTPS release manifest at startup, asks before every newly offered version, verifies the downloaded ZIP with SHA-256, registers Update/Repair/Uninstall in Windows Apps, and never installs an update after the learner chooses Later.
