...
export async function POST(request: Request) {
  ...
  if (provider === "deepl") {
    ...
    const apiKey = String(body.apiKey || "").trim().slice(0, 1000) || current?.apiKey || "";
    const tier = String(body.tier || current?.tier || (apiKey.endsWith(":fx") ? "free" : "pro")) === "pro" ? "pro" : "free";
    ...
    await saveProviderSecret(owner, "deepl", { apiKey, tier }, { keyHint: mask(apiKey), tier, message: "", ...result.metadata }, "connected", new Date().toISOString());
...
export async function DELETE(request: Request) {
  ...
  } else await deleteProviderSecret(owner, provider === "deepl" ? "deepl" : "openai");
