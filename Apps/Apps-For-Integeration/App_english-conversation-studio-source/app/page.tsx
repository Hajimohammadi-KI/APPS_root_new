...
const paths = ["Complete English", "Complete German"] as const;
...
export default function Home() {
  ...
  const language: StudioLanguage = path === "Complete German" ? "de" : "en";
  ...
  const [topicId, setTopicId] = useState("en-introducing-yourself");
  ...
  function selectPath(nextPath: (typeof paths)[number]) {
    const nextLanguage: StudioLanguage = nextPath === "Complete German" ? "de" : "en";
    const nextAll = copy[nextLanguage].all;
  ...
  function speak(value: string) {
    if (!("speechSynthesis" in window)) {
      setMessage(language === "de" ? "Sprachausgabe wird von diesem Browser nicht unterstützt." : "Speech playback is not supported by this browser.");
      return;
    }
    window.speechSynthesis.cancel();
      ...
      <main>
        ...
        <section className="conversation-filter" aria-labelledby="filter-heading">
          ...
          <div className="conversation-filter-grid">
            <label><span>{text.labels[0]}</span><select disabled={recordingState !== "idle"} value={path} onChange={(event) => selectPath(event.target.value as (typeof paths)[number])}>{paths.map((item) => <option key={item}>{item}</option>)}</select></label>
