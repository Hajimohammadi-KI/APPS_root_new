...
**CURRENT · Version2 0.7.0** — the Projekt-Fahrplan is a 37-week, capacity-based full-time plan rather than a fixed 25-week schedule. Its 185 active days preserve the medically protected rest and paper-only windows. Every regular day exposes one large collapsible guide for four hours of research or project learning plus four hours of implementation, testing, and documentation. The first 18 weeks assign each required paper an explicit 16-hour cycle across four days; the fifth day is closed-book teach-back, integration, and a traceable weekly output. Reading stops after one understood semantic unit, at most two necessary vocabulary items, three own sentences, and a source reference—never after an arbitrary page target. Existing project-day IDs remain stable so stored progress and notes are not silently attached to different work. The Windows installation continues to provide consent-based Update, Repair, and Uninstall while preserving learner data.
...
- `/` – Tagesdashboard, 37-Wochen-Vollzeitplan, Fortschritt und Fokus-Timer
