...
export function LearningRecoveryBoundary({children}:{children:ReactNode}) {
  ...
  if(error)return <main className="mx-auto max-w-xl p-8" role="alert"><h1>Lerndaten müssen wiederhergestellt werden</h1><p>{error}</p><p>Schließe andere App-Tabs und lade diese Seite neu. Vorhandene Daten wurden beibehalten.</p><button type="button" onClick={()=>window.location.reload()}>Wiederherstellung erneut versuchen</button></main>;
  if(!ready)return <p role="status" className="p-8">Gespeicherte Lerndaten werden geprüft…</p>;
  return children;
}
